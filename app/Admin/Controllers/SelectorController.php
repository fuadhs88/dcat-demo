...
class SelectorController extends Controller
{
    ...
    protected function grid()
    {
        return Grid::make(new TeaTable(), function (Grid $grid) {
            $grid->id('ID')->bold();
            $grid->name('Name');
            $grid->norms('specification');
            $grid->category('category')->label('default');
            $grid->price('price')->display(function ($value) {
                return number_format($value / 1000, 2);
            });
            $grid->shop_name('shop')->link();
            $grid->brand('brand')->link();
            $grid->state('上架?')->switch();
            $grid->added_at('Added time');
            ...
            $grid->selector(function (Grid\Tools\Selector $selector) {
                $selector->select('brand', 'Brand', ['AiW', 'All home', 'YaLM', 'Sweet Dreams', 'Fanai Furniture', '偶堂家私']);
