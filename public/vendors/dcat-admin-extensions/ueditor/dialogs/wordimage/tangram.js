 ...
 * @param {Function} iterator 对每个数组元素进行调用的函数，该函数有两个参数，第一个为数组元素，第二个为数组index 值，function (item, index)。
...
/**
 * operatingflash对象的方法，包括创建flash对象、获取flash对象以及判断flash插件的版本号
 * @namespace baidu.swf
 */
baidu.swf = baidu.swf || {};
 ...
 * @param {boolean} options.seamlesstabbing 	允许设置执行无缝跳格，从而使用户能跳出flashapplication程序。该参数只能在安装Flash7及更高版本的Windows中使用。允许值：true/false
 ...
 * @param {Object} 	options.vars 				要传递给flash的参数，支持JSON或string类型。
 * 
 * @param {HTMLElement|string} [container] 		flash对象的父容器元素，不传递该参数时在当前Code位置创建flash对象。
 * @meta standard
 ...
 * @config {Number} vars.gridWidth 每一个Preview 图片所占的宽度，应该为flash寛的整除
 * @config {Number} vars.gridHeight 每一个Preview 图片所占的高度，应该为flash高的整除
 * @config {Number} vars.picWidth 单张Preview 图片的宽度
 * @config {Number} vars.picHeight 单张Preview 图片的高度
 * @config {String} vars.uploadDataFieldName POST请求中图片数据的key,Default值'picdata'
 * @config {String} vars.picDescFieldName POST请求中图片description的key,Default值'picDesc'
 ...
 * @param {String} listBacgroundkUrl layout 控件背景
